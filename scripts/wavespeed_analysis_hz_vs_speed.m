...
% fit the data point
P = polyfit(hz,speed,1);
yfit = P(1)*hz+P(2);

Rsq1 = 1 - sum((speed - yfit).^2)/sum((speed - mean(speed)).^2);

% fit the data point of low frequencies
P2 = polyfit(hz2,speed2,1);
yfit2 = P2(1)*hz2+P2(2);

Rsq12 = 1 - sum((speed2 - yfit2).^2)/sum((speed2 - mean(speed2)).^2);
...
plot(hz, speed, '^','markerfacecolor',[0 0 0],'markeredgecolor',[0 0 0],'MarkerSize',msz);
hold on;
plot(hz,yfit,'r-','LineWidth', alw);
hold on;
%plot(hz2,yfit2,'b-','LineWidth', alw);

% adjust figure properties

%xlabel('frequency (Hz)','FontSize', 6,'FontWeight','normal','Fontname','arial');
%ylabel('speed (m/s)','FontSize', 6,'FontWeight','normal','Fontname','arial');
...
%t2 = text(35,0.45,['speed = ' num2str(roundn(P(1),-4)) ' * frequency + ' num2str(roundn(P(2),-4)) ]);
t3 = text(35,0.55,['rsq = ' num2str(roundn(Rsq1,-3))],'FontSize', fsz);
t4 = text(35,0.65,['speed = ' num2str(roundn(P(1),-3)) ' * frequency + ' num2str(roundn(P(2),-3)) ],'FontSize', fsz);
%t3 = text(35,0.55,['rsq = ' num2str(roundn(Rsq12,-4))]);
%t4 = text(35,0.65,['speed = ' num2str(roundn(P2(1),-4)) ' * frequency + ' num2str(roundn(P2(2),-4)) ]);
...
print(h1,'-dpng','wavespeed_frequency_frequency_vs_speed.png','-r1000');
